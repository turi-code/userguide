...
immutable; in the following snippet, `add_vertices` and `add_edges` both return
a new graph.
...
attributes. For this example we download a dataset of James Bond characters to
...
be done with the `SGraph.summary`---which prints the number of vertices and
